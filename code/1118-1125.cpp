...
typedef struct gongxiang{
    long read,send,web,log;
}sharemem;
...
void web(int fd, int hit,long timeuse) 
{ 
  struct timeval trs,tre,tss,tse,tws,twe,tls,tle;
  ...
  timeuse = (tre.tv_sec-trs.tv_sec)*1000000+tre.tv_usec-trs.tv_usec;
  ...
  /*timeuse[1] = (tse.tv_sec-tss.tv_sec)*1000000+tse.tv_usec-tss.tv_usec;*/
 /*判断客户端HTTP请求消息是否为GET类型，如果不是则给出相应的响应消息*/ 
  ...
  /*timeuse[3] = (tle.tv_sec-tls.tv_sec)*1000000+tle.tv_usec-tls.tv_usec;*/
 ...
 gettimeofday(&tws,NULL);
  ...
  /*timeuse[2] = (twe.tv_sec-tws.tv_sec)*1000000+twe.tv_usec-tws.tv_usec;*/
...
{ 
  ...
  socklen_t length; 
  double sumrt=0,sumst=0,sumlt=0,sumwt=0;
  long timeuse;
  long avgtime;
  ...
  int shm_fd1; //创建共享内存对象
  if((shm_fd1=shm_open(SHM_NAME,O_RDWR| O_CREAT,0666)) < 0){ 
    perror("create shared memory object error");
    exit(1); 
  }
  int shm_fd2; //创建共享内存对象
  if((shm_fd2=shm_open(SHM_NAME,O_RDWR| O_CREAT,0666)) < 0){ 
    perror("create shared memory object error");
    exit(1); 
  }
  int shm_fd3; //创建共享内存对象
  if((shm_fd3=shm_open(SHM_NAME,O_RDWR| O_CREAT,0666)) < 0){ 
    perror("create shared memory object error");
    exit(1); 
  }
  int shm_fd4; //创建共享内存对象
  if((shm_fd4=shm_open(SHM_NAME,O_RDWR| O_CREAT,0666)) < 0){ 
    perror("create shared memory object error");
    exit(1); 
  }
  /* 配置共享内存段大小*/
  ftruncate(shm_fd1, sizeof(int)); //将共享内存对象映射到进程
  void * memPtr = mmap(NULL, sizeof(sharemem), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd1, 0);
   /*ftruncate(shm_fd2, sizeof(int)); //将共享内存对象映射到进程
  void * memPtsend = mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd2, 0);
   ftruncate(shm_fd3, sizeof(int)); //将共享内存对象映射到进程
  void * memPtweb = mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd3, 0);
   ftruncate(shm_fd4, sizeof(int)); //将共享内存对象映射到进程
  void * memPtlog = mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd4, 0);*/
  ...
  for(hit=1; ;hit++) { 
    ...
    {
      web(socketfd,hit,timeuse);
      sem_wait(psem);    //进入临界区
      *(unsigned long *)memPtr += timeuse;
      avgtime = *(unsigned long *)memPtr/hit;
      printf("Average reading time is %ld\n",avgtime);
